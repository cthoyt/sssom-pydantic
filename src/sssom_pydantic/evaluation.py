...
from curies.vocabulary import exact_match as EXACT_MATCH, lexical_matching_process as LEXICAL_MATCH
...
def evaluate_predictions(
    *,
    positive: Iterable[SemanticMapping],
    negative: Iterable[SemanticMapping],
    predicted: Iterable[SemanticMapping],
    tag: str | None = None,
):
...
def grounder_to_mappings(grounders: dict[str, Grounder]) -> list[SemanticMapping]:
    ...
    mappings = [
        SemanticMapping(subject=s, predicate=EXACT_MATCH, objecto=o, justification=lexical_matching_process, confidence=confidence)
        for s, o, confidence in _grounder_to_mappings(grounders)
    ]
    mappings = assemble_evidences(mappings, progress=False)
    return mapping_set, mappings
